...
import { buildForDev, serverLocation } from "../config";
...
class LoginForm extends React.Component {
  ...
  render() {
    ...
    const { awaitingCreateResponse, termsAccepted } = this.state;
    return (
      <div
        className={
          "modal-wrapper" +
          (loggedInFacebook && loggedInApp ? " modal-fade-out" : "")
        }
      >
        <FadeTransform
          in
          transformProps={{
            enterTransform: "translateY(1.5rem)",
            exitTransform: "translateY(-1.5rem)",
          }}
        >
            ...
            <div className="modal-inner-container">
              <div className="login-form-container">
                ...
                {loggedInFacebook &&
                  !loggedInApp &&
                  !createAccount &&
                  !loginError && <p>Logging in...</p>}

                {loginError && (
                  <p className="login-error-message">
                    Login error. See developer console for information.
                  </p>
                )}
                ...
                {buildForDev && (
                  <p style={{ color: "red" }}>
                    Dev mode enabled - facebook login button is disabled.
                  </p>
                )}
                ...
                {createAccount && (
                  <div className="login-create-container">
                    <p>
                      Please accept the terms and click Confirm to finish
                      setting up your account
                    </p>
                    <label className="login-create-label">Name:</label>
                    <input
                      type="text"
                      className="login-create-textinput"
                      value={name}
                      disabled
                    ></input>
                    ...
                    <button
                      ...
                      disabled={
                        !termsAccepted || awaitingCreateResponse || email == ""
                      }
                    >
                ...
                {responseErrorMessage != "" && (
                  <p className="login-error-text">{responseErrorMessage}</p>
                )}

                {!loggedInFacebook && (
                  <p className="modal-large-text">
                    Please log in using Facebook to continue.
                  </p>
                )}
                {!loggedInFacebook && (
                  <p className="modal-disclaimer-text">
                    We do not store any of your personal information except your
                    email address, which is used only to alert you about your
                    devices if you turn alerts on. We will not share your
