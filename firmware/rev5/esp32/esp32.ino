...
String webPassword = "jiblet123";
String webDeviceID = "1";
char* wifiSSID = "TanglesExtended";
char* wifiPassphrase =  "Jiblet!1337";

#define webEndpoint "https://noobs.wtf/plants/api.php" //the php script that the ESP8266 will connect to for uploading data
...
void flashRead() {
  configUploadFrequencyMins      = preferences.getUInt("uploadFreq", 1);
  Serial.print("Read upload frequency from flash: ");
  Serial.println(configUploadFrequencyMins);
  //temp 1
...
void flashWrite() {
  preferences.putUInt("uploadFreq", configUploadFrequencyMins);
  //temp 1
...
void handleReceiveConfig(String webResponse) {
  ...
  configUploadFrequencyMins = doc["configUploadFrequencyMins"];
...
void tempCheckState() {
  if (configTemp1Enable) {
    ...
    if (isnan(humidity1)) { humidity1 = 0.0; }
  }
  if (configTemp2Enable) {
    temp2 = dht2.readTemperature();
    humidity2 = dht2.readHumidity();
    if (isnan(temp2)) { temp2 = 0.0; }
    if (isnan(humidity2)) { humidity2 = 0.0; }
  }
}
...
unsigned int pump2MeasuredFlowMl = 0;
...
void uploadData(int curMillis) {
  ...
  if (wifiConnected) {
    if (curMillis - lastDataUpload > configUploadFrequencyMins * 1000 * 60 || !firstUpload) {
      Serial.print("Uploading data. Last upload ");
      ...
      uploadData += "&a="; uploadData += !firstUpload && alarmReason == "Sync Error" ? "Sync OK" : alarmReason;
      ...
      if(http.begin(webEndpoint)){
        ...
        String postString = "user=" + webUsername + "&password=" + webPassword + "&deviceid=" + webDeviceID + "&" + uploadData;
